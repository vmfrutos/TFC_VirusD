...
CharacterControllerPhysics::CharacterControllerPhysics(btPairCachingGhostObject * ghostObject, btConvexShape * convexShape, btScalar stepHeight,
		btCollisionWorld * collisionWorld, btVector3 pos,int upAxis)
{
	mUpAxis = upAxis;
	mAddedMargin = 0.02;
...
{
	std::cout << "recoverFromPenetration: " <<  std::endl;
...
void CharacterControllerPhysics::updateAction(btCollisionWorld * collisionWorld, btScalar dt)
{
	...
	//std::cout << "m_allowedCcdPenetration: " << Ogre::StringConverter::toString(collisionWorld->getDispatchInfo().m_allowedCcdPenetration) << std::endl;
}
