...
pLogManager->logMessage("Se va a cargar " +_path);
	...
	pLogManager->logMessage("Se ha cargado el track");
...
(bool loop)
{
	...
	if(Mix_PausedMusic()) {// Estaba pausada?
		...
		if (Mix_PlayMusic(_pTrack,loop?-1:0) == -1) {
		...
		pLogManager->logMessage("Se ha llamado a Mix_PlayMusic");
	}
}
...
Track::stop()
{
	Mix_HaltMusic();
	Ogre::LogManager* pLogManager = Ogre::LogManager::getSingletonPtr();
	pLogManager->logMessage("Se ha llamado a Mix_HaltMusic");
}
